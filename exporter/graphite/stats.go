...
// statsExporter exports stats to the Graphite Monitoring.
...
var (
	errBlankProjectNamespace = errors.New("expecting a non-blank ProjectNamespace")
	errSingletonExporter     = errors.New("only one exporter can be created per unique ProjectNamespace per process")
)

// newStatsExporter returns an exporter that uploads stats data to Graphite.
// Only one Graphite exporter should be created per ProjectNamespace per process, any subsequent
// invocations of NewExporter with the same ProjectNamespace will return an error.
func newStatsExporter(o Options) (*statsExporter, error) {
	if strings.TrimSpace(o.ProjectNamespace) == "" {
		return nil, errBlankProjectNamespace
	}

	if strings.TrimSpace(o.GraphiteEndpoint) == "" {
		// Setting default endpoint
		o.GraphiteEndpoint = "127.0.0.1:8125"
	}

	seenProjectsMu.Lock()
	defer seenProjectsMu.Unlock()
	_, seen := seenProjects[o.ProjectNamespace]
	if seen {
...
// ExportView exports to the Graphite if view data
		...
		for _, row := range vd.Rows {
			...
			if len(timeSeries) == limit {
				reqs = append(reqs, &monitoringpb.CreateTimeSeriesRequest{
					Name:       monitoring.MetricProjectPath(e.o.ProjectNamespace),
...
func (e *statsExporter) createMeasure(ctx context.Context, vd *view.Data) error {
	...
	md, err := createMetricDescriptor(ctx, e.c, &monitoringpb.CreateMetricDescriptorRequest{
		Name: fmt.Sprintf("projects/%s", e.o.ProjectNamespace),
		MetricDescriptor: &metricpb.MetricDescriptor{
			Name:        fmt.Sprintf("projects/%s/metricDescriptors/%s", e.o.ProjectNamespace, metricType),
...
func equalMeasureAggTagKeys(md *metricpb.MetricDescriptor, m stats.Measure, agg *view.Aggregation, keys []tag.Key) error {
	...
	case metricpb.MetricDescriptor_INT64:
		if _, ok := m.(*stats.Int64Measure); !(ok || agg.Type == view.AggTypeCount) {
			return fmt.Errorf("graphite metric descriptor was not created as int64")
		}
	...
	case metricpb.MetricDescriptor_DOUBLE:
		if _, ok := m.(*stats.Float64Measure); !ok {
			return fmt.Errorf("graphite metric descriptor was not created as double")
		}
	...
	if !aggTypeMatch {
		return fmt.Errorf("graphite metric descriptor was not created with aggregation type %T", agg.Type)
	}

	if len(md.Labels) != len(keys)+1 {
		return errors.New("graphite metric descriptor was not created with the view labels")
	}
	...
	for _, k := range md.Labels {
		if _, ok := labels[k.Key]; !ok {
			return fmt.Errorf("graphite metric descriptor was not created with label %q", k)
		}
	}
